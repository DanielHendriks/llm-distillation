...
OUTPUTS_DIR = os.path.join(SCRIPT_DIR, '..', 'llm_outputs', 'rationales_revised')


class RevisionProcessHandler(ProcessHandler):
    def __init__(self, checkpoint, dataset_handler: CQADatasetLoader, max_new_tokens, **kwargs):
        self.dataset_handler = dataset_handler
        super().__init__(checkpoint, max_new_tokens)
    
    def revise(self, input_name, output_name, batch_size, n_examples_revision, include_true_answer, resume_from_shard=None):
        self.dataset_handler.prepare_for_revision(input_name=input_name, n_examples_revision=n_examples_revision, include_true_answer=include_true_answer)
        assert self.dataset_handler.ready_for_revision == True
        revision_column = 'few_shot_revision_prompt'
        ...
        if self.dataset_handler.sample == 1: 
            ...
            for shard_idx in tqdm(range(start_shard, _n_shards), desc="Processing shards"):
                ...
                for batch in tqdm(data_loader):
                    revisions_extracted = self._batch_generate(batch, revision_column)
                    all_revisions_list.extend(revisions_extracted)
                        
                # save revisions to disk
                shard = shard.add_column('few_shot_revision', all_revisions_list)
            ...
            self.dataset_handler.dataset = self.dataset_handler.dataset.add_column('few_shot_revision', all_revisions_list) 
...
def main(args):
    ...
    # create RevisionProcessHandler
    process_handler = RevisionProcessHandler(
    ...
    process_handler.revise(
        ...
        output_name=args.output_name,   # folder to store revisions
        batch_size=args.batch_size, 
        n_examples_revision=args.n_examples_revision,
        include_true_answer=args.include_true_answer,
...
if __name__ == "__main__":
    ...
    parser.add_argument('--include_true_answer', action='store_false', default=True)
    parser.add_argument('--n_examples_revision', type=int, default=4)
    parser.add_argument('--checkpoint', type=str, default='meta-llama/Llama-2-13b-chat-hf')
